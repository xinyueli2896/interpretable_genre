...
class ModelOutput:
    def __init__(
        self,
        recon_program_logits=None,
        recon_pitchdur_logits=None,
        mu=None,
        logvar=None,
        z=None,
        z_concept=None,
...
class VAEConceptModel(nn.Module):
    ...
    def __init__(
        ...
        if self.use_concept_bottleneck:
            ...
            classifier_input = num_concepts
        else:
            self.latent_to_concept = None
            self.concept_to_latent = None
            classifier_input = latent_dim

        # --- Classifier (song-level via mean over measures) ---
    ...
    def forward(self, rolls, mask):
        ...
        if pred_concepts is not None:
            ...
            concepts_bm = None
            concept_latent_bm = None
            class_logits = self.classifier(z_bm.mean(dim=1))  # (B, num_genres)

...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Predict genre from latent z directly (no concept bottleneck).",
    )
    parser.add_argument(
        "--no_reconstruction",
        action="store_true",
        help="Disable decoder and reconstruction loss.",
    )
    parser.add_argument(
        "--no_concept_alignment",
        action="store_true",
        help="Disable concept-latent alignment loss.",
    )
    parser.add_argument(
        "--no_concept_supervision",
        action="store_true",
        help="Disable concept supervision loss.",
    )
    ...
    if args.no_concept_supervision:
        args.concept_weight = 0.0
    if args.no_concept_alignment:
        args.concept_recon_weight = 0.0
    if args.no_reconstruction:
        args.recon_weight = 0.0
        if args.recon_only:
            args.recon_only = False
    if args.no_concept_bottleneck:
        args.concept_weight = 0.0
        args.concept_recon_weight = 0.0
    ...
    input_dim = 0
    use_concept_bottleneck = not args.no_concept_bottleneck
    enable_decoder = not args.no_reconstruction
    ...
    for epoch in range(args.epochs):
        ...
        for step_idx, batch in enumerate(loader, start=1):
            ...
            decoder_enabled = (
                output.recon_program_logits is not None and output.recon_pitchdur_logits is not None
            )
            recon = torch.tensor(0.0, device=device)
            program_target = None
            pitchdur_target = None
            if decoder_enabled:
            ...
            if output.concepts is not None:
            ...
            if output.concept_to_latent is not None and output.z_concept is not None:
            ...
            if wandb_run is not None and rank == 0 and global_step % args.wandb_train_log_every == 0:
                with torch.no_grad():
                    ...
                    recall = 0.0
                    f1 = 0.0
                    if decoder_enabled and pitchdur_target is not None:
